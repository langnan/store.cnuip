...
@connect(({ consultation, user, loading }) => ({
  ...
  submitting: loading.effects['requirement/replyConsultation'],
}))
@Form.create({})
export default class Consultation extends React.Component {
  state = {
    tabKey: 1,
    breadcrumbList: [...breadcumbInit, { title: '商品咨询' }, { title: '咨询详情' }],
    detail: {},
  };
  ...
  reply = e => {
    ...
    form.validateFieldsAndScroll((err, fieldsValue) => {
      if (err) return;
      dispatch({
        type: 'consultation/replyConsultation',
        payload: {
          goodsCommentId: detail.id,
